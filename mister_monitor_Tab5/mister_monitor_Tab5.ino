  ...
  if (g_fineActive) {
    ...
    for (int dy = dyFirst; dy <= dyLast; dy++) {
      ...
      if (outX >= 0 && outY >= 0 &&
        ...
        M5.Display.pushImage(outX, outY, runLen, 1, g_fineRow);
      }
    }
    return 1;
  }
...
bool displayCoreImageCentered(String imagePath) {
  ...
  // Decode with automatic centering. Reading from the card removes the need for
  // one contiguous block the size of the image -- psramMalloc() asks for
  // MALLOC_CAP_INTERNAL despite its name, so this board was subject to the same
  // fragmentation ceiling as the CYDs, just with more headroom before it bit.
