...
#pragma once
#include <Arduino.h>
#include "board_hal.h"     // SD (SDMMC) comes from the board HAL
...
struct AppConfig {
  ...
  // board upside down. Only 0 and 180 are offered: 90/270 would leave the
  // panel in portrait and the UI is laid out on a fixed landscape grid.
