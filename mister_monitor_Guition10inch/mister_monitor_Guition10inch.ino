...
#define DISPLAY_BRIGHTNESS_NORMAL   204     // backlight level outside standby
...
// Hardware constants. The HAL exposes a 1280x720 canvas, centred on the
// 1280x800 panel, so every coordinate below is in that canvas.
...
#define SCALE_X           ((float)UI_WIDTH  / ORIGINAL_WIDTH)
#define SCALE_Y           ((float)UI_HEIGHT / ORIGINAL_HEIGHT)
...
// Five rows: the fullscreen layout fits five 110-px rows in the 550-px band
// under the title rule. The server pages by the per= we send, so this is a
// firmware-only decision.
const int RA_LIST_PER_PAGE = 5;

// Trophy-row touch bands, in PHYSICAL panel coordinates (Board.Touch reports
// 1280x720 directly) and DERIVED from the fullscreen list layout:
// displayRAList() draws the rows at y = 145 + i*100, so row i occupies the
// band 145+100i .. 245+100i and is centred at 195 + i*100.
//
// The handler turns these into the split between the two page-6 gestures:
// everything above (TOP - PITCH/2) = 145 is the header, whose tap cycles
// subpages, and everything below maps to a row. Raising TOP therefore widens
// the header target; the two numbers are not independent of the draw layout.
//
// Verify with the serial line the page-6 handler prints on every tap: on a
// FULL (5-row) list page the five row centres should read
// ~195/295/395/495/595. The +/-50 px bands are finger-sized; override these
// two only if the panel consistently disagrees:
//   RA_ROW_TOUCH_TOP   = <first-row centre reading>
//   RA_ROW_TOUCH_PITCH = (<last-row reading> - <first-row reading>) / 4
int RA_ROW_TOUCH_TOP   = 195;  // physical touch Y at centre of the FIRST row
int RA_ROW_TOUCH_PITCH = 100;  // physical touch Y between consecutive row centres
int  raSubPage   = 0;
...
// setup(). STEP is no longer a compile-time constant so the user can slow the
// synopsis down; AUTO=false freezes it and the panel then dwells for
// GAMEINFO_SYN_FIT_MS (the "fits without scrolling" path) before returning to
// the image.
...
// 4 s bottom hold (GAMEINFO_SYN_PAUSE_MS), that is ~13 s of reading time on
// the final screenful — the one place where the reader is mid-sentence and
// cannot scroll back, so it is deliberately the longest hold in the cycle.
const unsigned long GAMEINFO_SYN_EXIT_MS  = 9000;  // tail after the last line
...
void handleScreenshotRoot() {
  ...
  html += "Resolution: " + String(Board.Display.width()) + "x" + String(Board.Display.height()) + " &nbsp;|&nbsp; ";
  html += "IP: " + ip + ":8080</p>";
  html += "</body></html>";
...
void handleScreenshot() {
  int w = Board.Display.width();   // 1280
  int h = Board.Display.height();  // 720
  ...
  for (int y = 0; y < h; y++) {
    ...
    Board.Display.readRect(0, y, w, 1, rowSrc);

    // Convert RGB565 → RGB888 in-place into rowDst
    // BMP stores pixels as B, G, R (reversed byte order)
    for (int x = 0; x < w; x++) {
      // readRect returns big-endian RGB565 — swap bytes before extracting
...
const int GI_VAL_X     = 260;  // value column origin (PHYSICAL: page 5 is
                               // fullscreen, so these are panel coordinates)
const int GI_VAL_CHARS = 41;   // 984 px at 24 px/char (size 4), ends at 1244
...
void drawGameInfoPageIndicator(bool pressed);
...
bool isFullscreenPage(int p);
void drawFullscreenChrome();
void drawFullscreenNavHints(int pressedZone);
bool handleFullscreenStripTouch(int physicalX);
void drawRAStatPanel(int x, int y, int w, int h, const char* label,
                     const String& value, uint16_t color);
int  drawWrappedTextPhys(const String& text, int x, int y, int charsPerLine,
                         int maxLines, uint16_t color, int textSize);
...
// positioned at offset (90, 200) on the 1280x720 canvas
...
public:
  ...
  void fillRect(int x, int y, int w, int h, uint16_t color) {
    Board.Display.fillRect(scaleX(x), scaleY(y), scaleW(w), scaleH(h), color);
  }
  
  void drawRect(int x, int y, int w, int h, uint16_t color) {
    Board.Display.drawRect(scaleX(x), scaleY(y), scaleW(w), scaleH(h), color);
  }
  ...
  void drawFastHLine(int x, int y, int w, uint16_t color) {
    Board.Display.drawFastHLine(scaleX(x), scaleY(y), scaleW(w), color);
  }
  
  void drawFastVLine(int x, int y, int h, uint16_t color) {
    Board.Display.drawFastVLine(scaleX(x), scaleY(y), scaleH(h), color);
  }
  
  void drawLine(int x0, int y0, int x1, int y1, uint16_t color) {
    Board.Display.drawLine(scaleX(x0), scaleY(y0), scaleX(x1), scaleY(y1), color);
  }
  
  void fillCircle(int x, int y, int r, uint16_t color) {
    Board.Display.fillCircle(scaleX(x), scaleY(y), (int)(r * SCALE_FACTOR), color);
  }
  
  void drawCircle(int x, int y, int r, uint16_t color) {
    Board.Display.drawCircle(scaleX(x), scaleY(y), (int)(r * SCALE_FACTOR), color);
  }
  
  // ========== TEXT OPERATIONS WITH AUTO-SCALING ==========
  
  void setCursor(int x, int y) {
    Board.Display.setCursor(scaleX(x), scaleY(y));
  }
  
  void setTextColor(uint16_t color) {
    Board.Display.setTextColor(color);
  }
  
  void setTextColor(uint16_t fg, uint16_t bg) {
    Board.Display.setTextColor(fg, bg);
  }
  
  void setTextWrap(bool wrap) {
    Board.Display.setTextWrap(wrap);
  }
  ...
  void setTextSize(float size) {
    ...
    Board.Display.setTextSize(size * SCALE_FACTOR);
  }
  
  void print(const char* text) {
    Board.Display.print(text);
  }
  ...
  void print(int num) {
    Board.Display.print(num);
  }
  
  void print(float num, int decimalPlaces = 2) {
    Board.Display.print(num, decimalPlaces);
  }
  
  void println(const char* text) {
    Board.Display.println(text);
  }
  ...
  void println(char c) {
    Board.Display.println(c);
  }
  
  void println(int num) {
    Board.Display.println(num);
  }
  
  void println(float num, int decimalPlaces = 2) {
    Board.Display.println(num, decimalPlaces);
  }
  ...
  void pushImage(int x, int y, int w, int h, uint16_t* data) {
    ...
    Board.Display.pushImage(x, y, w, h, data);
  }
};

// Create global instance of scaled display
// Drawing goes through this wrapper; images go straight to Board.Display.
ScaledDisplay Lcd;
...
void showDownloadingScreen(String coreName, String gameName) {
  ...
  Board.Display.fillScreen(THEME_BLACK);
  ...
  if (!logoLoaded) {
    ...
    Board.Display.setTextColor(THEME_WHITE);
    Board.Display.setTextSize(4);
    Board.Display.setCursor(850, 180);
    Board.Display.print("MiSTer");
    Board.Display.setTextColor(THEME_YELLOW);
    Board.Display.setTextSize(2);
    Board.Display.setCursor(870, 230);
    Board.Display.print("FPGA");
  ...
  Board.Display.fillRect(PANEL_OFFSET_X, PANEL_OFFSET_Y, 640, 480, THEME_BLACK);
  
  // Header with ScreenScraper logo area
  Board.Display.setTextColor(THEME_CYAN);
  Board.Display.setTextSize(4);  // Logical size 2 = physical size 4
  Board.Display.setCursor(PHYS_X(40), PHYS_Y(5));
  Board.Display.print("SCREENSCRAPER");
  
  Board.Display.setTextColor(THEME_GREEN);
  Board.Display.setTextSize(2);  // Logical size 1 = physical size 2
  Board.Display.setCursor(PHYS_X(65), PHYS_Y(25));
  Board.Display.print("AUTO-DOWNLOAD");
  
  // Decorative line
  Board.Display.drawFastHLine(PHYS_X(20), PHYS_Y(40), 560, THEME_CYAN);  // 280*2=560
  
  // Main download panel (using drawPanel would require modifying it; draw rect directly)
  Board.Display.drawRect(PHYS_X(10), PHYS_Y(48), 600, 180, THEME_BLUE);  // 300*2=600, 90*2=180
  Board.Display.fillRect(PHYS_X(10)+2, PHYS_Y(48)+2, 596, 176, THEME_BLACK);
  
  // Download status title
  Board.Display.setTextColor(THEME_WHITE);
  Board.Display.setTextSize(2);
  Board.Display.setCursor(PHYS_X(20), PHYS_Y(58));
  Board.Display.print("DOWNLOADING GAME ARTWORK");
  
  // Separator
  Board.Display.drawFastHLine(PHYS_X(20), PHYS_Y(70), 560, THEME_GRAY);
  
  // Core info
  Board.Display.setTextColor(THEME_CYAN);
  Board.Display.setTextSize(2);
  Board.Display.setCursor(PHYS_X(20), PHYS_Y(78));
  Board.Display.print("CORE:");
  
  Board.Display.setTextColor(THEME_YELLOW);
  Board.Display.setCursor(PHYS_X(65), PHYS_Y(78));
  String coreFold = foldForDisplay(coreName);   // display-only fold
  String displayCore = coreFold.length() > 20 ? coreFold.substring(0, 20) + "..." : coreFold;
  Board.Display.print(displayCore);
  
  // Game info
  Board.Display.setTextColor(THEME_CYAN);
  Board.Display.setCursor(PHYS_X(20), PHYS_Y(93));
  Board.Display.print("GAME:");
  
  Board.Display.setTextColor(THEME_YELLOW);
  Board.Display.setCursor(PHYS_X(65), PHYS_Y(93));
  String displayGame = gameName.length() > 20 ? gameName.substring(0, 20) + "..." : gameName;
  Board.Display.print(displayGame);
  
  // Status
  Board.Display.setTextColor(THEME_GREEN);
  Board.Display.setTextSize(2);
  Board.Display.setCursor(PHYS_X(20), PHYS_Y(113));
  Board.Display.print("STATUS: Searching database...");
...
void showDownloadProgressColored(int progress, String text, uint16_t barColor) {
  ...
  Board.Display.fillRect(PHYS_X(10), PHYS_Y(130), 600, 180, THEME_BLACK);
  
  // Progress bar
  Board.Display.drawRect(PHYS_X(20), PHYS_Y(140), 560, 40, THEME_WHITE);
  Board.Display.fillRect(PHYS_X(20)+2, PHYS_Y(140)+2, 556, 36, THEME_BLACK);
  
  // Fill progress bar
  if (progress > 0) {
    int fillWidth = (progress * 552) / 100;  // 276*2 = 552
    Board.Display.fillRect(PHYS_X(20)+4, PHYS_Y(140)+4, fillWidth, 32, barColor);
  }
  
  // Progress percentage in center
  Board.Display.setTextColor(THEME_WHITE);
  Board.Display.setTextSize(2);
  Board.Display.setCursor(PHYS_X(148), PHYS_Y(145));
  Board.Display.printf("%d%%", progress);
  
  // Status text below progress bar
  Board.Display.setTextColor(THEME_CYAN);
  Board.Display.setTextSize(2);
  Board.Display.setCursor(PHYS_X(20), PHYS_Y(170));
  
  // Truncate text if too long
  String displayText = text.length() > 38 ? text.substring(0, 38) + "..." : text;
  Board.Display.print(displayText);
  
  // Footer
  Board.Display.setTextColor(THEME_GRAY);
  Board.Display.setTextSize(2);
  Board.Display.setCursor(PHYS_X(20), PHYS_Y(200));
  Board.Display.print("Source: ScreenScraper.fr");
...
void addGameImageFooter(String gameName) {
  ...
  Board.Display.drawFastHLine(0, 620, 1280, THEME_GREEN);
  Board.Display.fillRect(0, 621, 1280, 99, THEME_BLACK);
  ...
  Board.Display.setTextColor(THEME_YELLOW, THEME_BLACK);  // bg color → flicker-free
  Board.Display.setTextSize(3);
  Board.Display.setCursor(198, 638);
  Board.Display.print(displayGame);
  
  // Instructions (lower line of footer) — shifted left when button is visible
  Board.Display.setTextColor(THEME_GREEN);
  Board.Display.setTextSize(3);
  Board.Display.setCursor(234, 685);
  Board.Display.print("Touch the screen to show MiSTer monitor");
...
//
// PHYSICAL coordinates: the image footer is drawn directly on Board.Display
// (band y 620..720 of the 1280x720 canvas), so the button lives there too.
...
void drawGameInfoIcon(bool pressed) {
  ...
  Board.Display.fillRect(BX, BY, BW, BH, bg);

  // "GAME INFO": 9 chars at size 3 = 162 px wide, 24 px tall — centred (19 px pad)
  Board.Display.setTextWrap(false);
  Board.Display.setTextColor(THEME_BLACK, bg);
  Board.Display.setTextSize(3);
  Board.Display.setCursor(BX + (BW - 162) / 2, BY + (BH - 24) / 2);
  Board.Display.print("GAME INFO");
}
...
void drawCoreImageFooter() {
  ...
  if (hasGame) {
    ...
    // Game name
    Board.Display.setTextColor(THEME_YELLOW, THEME_BLACK);
    Board.Display.setTextSize(3);
    Board.Display.setCursor(198, 638);
    Board.Display.print(displayGame);
    ...
    Board.Display.setTextColor(THEME_GREEN);
    Board.Display.setTextSize(3);
    Board.Display.setCursor(250, 660);
    Board.Display.print("Touch the screen to show MiSTer monitor");
  }
...
void drawFooter() {
  ...
  // Draw separator line at top of footer
  // Board.Display.drawFastHLine(0, 215 * SCALE_Y, UI_WIDTH, THEME_GREEN);
  
  // Fill footer background
  Board.Display.fillRect(0, 216 * SCALE_Y, UI_WIDTH, 24 * SCALE_Y, THEME_BLACK);
  
  // System uptime on the left
  Board.Display.setTextColor(THEME_CYAN);
  Board.Display.setTextSize(2);
  Board.Display.setCursor(10 * SCALE_X, 220 * SCALE_Y);
  Board.Display.printf("SYS: %02d:%02d", (millis() / 60000) % 60, (millis() / 1000) % 60);
  ...
  Board.Display.setTextWrap(false);
  Board.Display.setTextSize(2);
  Board.Display.setTextColor(THEME_CYAN, THEME_BLACK);
  ...
  if (!isFullscreenPage(currentPage)) {
    if (currentGame.length() > 0 && !g_currentGameIsContainer) {
      ...
      if (gameFooterScroll.fullText != gameSeed) {
        initScrollText(&gameFooterScroll, gameSeed, FOOTER_VISIBLE_CHARS);
      }
      ...
      Board.Display.setCursor(FOOTER_PHY_X, FOOTER_PHY_Y);
      Board.Display.printf("GAME: %s", displayGame.c_str());
    } else {
      String coreSeed = foldForDisplay(currentCore);   // display-only fold
      if (gameFooterScroll.fullText != coreSeed) {
        initScrollText(&gameFooterScroll, coreSeed, FOOTER_VISIBLE_CHARS);
      }
      String displayCore = padToWindow(getScrolledText(&gameFooterScroll), FOOTER_VISIBLE_CHARS);
    
      Board.Display.setCursor(FOOTER_PHY_X, FOOTER_PHY_Y);
      Board.Display.printf("CORE: %s", displayCore.c_str());
    }
  }
  
  // SD card status indicator on the right
  if (sdCardAvailable) {
    Board.Display.setCursor(250 * SCALE_X, 220 * SCALE_Y);
    Board.Display.print("IMG:OK");
  }
}
...
RomDetails getCurrentRomDetails() {
  ...
  for (int attempt = 1; attempt <= ROM_DETAILS_MAX_ATTEMPTS; attempt++) {
    ...
    if (code == 200) {
      ...
      // retry budget on every 0MHz load, and the name-search fallback (or the
      // container gate) takes over immediately.
...
RomDetails getCurrentRomDetailsForced() {
  ...
  for (int attempt = 1; attempt <= ROM_DETAILS_MAX_ATTEMPTS; attempt++) {
    ...
    if (code == 200) {
      ...
      // no_hash / container_image / ss_romnom, so SCAN decided from a details
      // struct that was blind to the newer server verdicts.
      if (!_parseRomDetailsJson(response, details, "Forced ")) {
        http.end();
        Serial.printf("=== FORCED ROM DETAILS COMPLETE (MEMORY FAIL) ===\n\n");
        return details;
      }
...
void ssNotifyOnce(const String& key, const char* message, const String& detail) {
  ...
  Board.Display.setTextSize(2);
  Board.Display.setCursor(PHYS_X(65), PHYS_Y(25));
  Board.Display.print("ARTWORK STATUS");

  Board.Display.drawFastHLine(PHYS_X(0), PHYS_Y(38), 640, THEME_YELLOW);

  Board.Display.setTextColor(THEME_WHITE);
  Board.Display.setTextSize(3);
  Board.Display.setCursor(PHYS_X(10), PHYS_Y(55));
  Board.Display.print(message);

  if (detail.length() > 0) {
    String d = detail.length() > 38 ? detail.substring(0, 38) + "..." : detail;
    Board.Display.setTextColor(THEME_YELLOW);
    Board.Display.setTextSize(2);
    Board.Display.setCursor(PHYS_X(10), PHYS_Y(80));
    Board.Display.print(d);
  }

  Board.Display.setTextColor(THEME_GREEN);
  Board.Display.setTextSize(2);
  Board.Display.setCursor(PHYS_X(10), PHYS_Y(105));
  Board.Display.print("Showing default image instead...");
...
void showCoreDownloadingScreen(String coreName) {
  ...
  Board.Display.setTextColor(THEME_ORANGE);
  Board.Display.setTextSize(2);  // Logical size 1 = physical size 2
  Board.Display.setCursor(PHYS_X(60), PHYS_Y(25));
  Board.Display.print("SYSTEM DOWNLOAD");
  
  // Decorative line
  Board.Display.drawFastHLine(PHYS_X(20), PHYS_Y(40), 560, THEME_CYAN);
  
  // Main download panel (orange for system vs blue for game)
  Board.Display.drawRect(PHYS_X(10), PHYS_Y(48), 600, 180, THEME_ORANGE);
  Board.Display.fillRect(PHYS_X(10)+2, PHYS_Y(48)+2, 596, 176, THEME_BLACK);
  ...
  Board.Display.setTextSize(2);
  Board.Display.setCursor(PHYS_X(20), PHYS_Y(58));
  Board.Display.print("DOWNLOADING SYSTEM ARTWORK");
  ...
  Board.Display.setTextSize(2);
  Board.Display.setCursor(PHYS_X(20), PHYS_Y(78));
  Board.Display.print("SYSTEM:");
  
  Board.Display.setTextColor(THEME_YELLOW);
  Board.Display.setCursor(PHYS_X(75), PHYS_Y(78));
  String coreFold = foldForDisplay(coreName);   // display-only fold
  String displayCore = coreFold.length() > 18 ? coreFold.substring(0, 18) + "..." : coreFold;
  Board.Display.print(displayCore);
  
  // Media types
  Board.Display.setTextColor(THEME_CYAN);
  Board.Display.setCursor(PHYS_X(20), PHYS_Y(93));
  Board.Display.print("MEDIA TYPES:");
  
  Board.Display.setTextColor(THEME_WHITE);
  Board.Display.setTextSize(2);
  Board.Display.setCursor(PHYS_X(20), PHYS_Y(106));
  Board.Display.print("screen marquee > photo > wheel");
    Serial.println("SD not available, showing SD error");
  // Status
  Board.Display.setTextColor(THEME_GREEN);
  Board.Display.setTextSize(2);
  Board.Display.setCursor(PHYS_X(20), PHYS_Y(123));
  Board.Display.print("STATUS: Searching database...");
...
// Touch replaces physical buttons on this board.
// It converts touch coordinates and checks which button (if any) was pressed

// Bottom-strip touch for the fullscreen pages (5 and 6): the CYD28 zone
// pattern, left/centre/right thirds. Returns true when it handled the touch.
// These pages do not draw the frame02 button column, so this is the ONLY way
// off them — every page-5/6 touch below y=648 belongs to their content.
bool handleFullscreenStripTouch(int physicalX) {
  int zone = (physicalX < 427) ? 0 : (physicalX < 853 ? 1 : 2);

  if (zone == 0 || zone == 2) {
    Serial.printf("  -> fullscreen strip %s\n", zone == 0 ? "PREV" : "NEXT");
    drawFullscreenNavHints(zone);       // white flash, strip feedback idiom
    if (zone == 0) playPrevButtonSound(); else playNextButtonSound();
    delay(150);
    currentPage = (zone == 0) ? (currentPage - 1 + totalPages) % totalPages
                              : (currentPage + 1) % totalPages;
    backgroundLoaded = false;           // leaving a fullscreen page: the target
                                        // reloads frame02 (or the next chrome)
    needsRedraw     = true;
  ...
  // --- SCAN ---------------------------------------------------------------
  if (scanInProgress) {
    Serial.println("  -> fullscreen strip SCAN (IGNORED — already scanning)");
    lastButtonPress = millis();
    return true;
  }

  // Same sequence as the generic SCAN handler, minus the frame02 button-label
  // dance: the white SCAN hint doubles as the busy cue while the refresh runs.
  Serial.println("  -> fullscreen strip SCAN (global refresh + force ROM rescan)");
  scanInProgress = true;
  drawFullscreenNavHints(1);
  playScanButtonSound();
  ...
  // No game: stay on the page and refresh what it shows.
  if (currentPage == 6) {
    getRAStatus();                     // trophies refetched on the spot
    lastRAFetch = millis();
  }
  scanInProgress = false;
  drawFullscreenNavHints(-1);
...
void handleTouch() {
  // Step 1: Get current touch state from the HAL
  ...
  if (touch.wasPressed()) {
    ...
    // Step 4: Convert to logical coordinates (320x240 space) if needed
    ...
    // GAME INFO page (5): fullscreen layout with the shared nav strip, so this
    // branch swallows EVERY page-5 touch — the generic PRV/SCAN/NXT hitboxes
    // below belong to the frame02 button column, which this page no longer
    // draws.
    if (currentPage == 5) {
      if (physicalY >= 648) {
        handleFullscreenStripTouch(physicalX);

      } else if (currentMeta.loaded && currentMeta.synopsis.length() > 0) {
        // Subpage toggle. The "1/2>>" indicator is the visual affordance, but
        // the hit target is the WHOLE content band above the strip: page 5 has
        // no other interactive element there, and one generous bound keeps it
        // immune to sampling hiccups and axis quirks. Gated on a synopsis
        // existing, because that is exactly when the indicator is drawn.
        ...
        drawGameInfoPageIndicator(true);
        playNextButtonSound();
        delay(200);
        ...
        Serial.println("  -> GAME INFO content tap (no synopsis) — ignored");
        lastButtonPress = millis();
      }
    }

    // RETROACHIEVEMENTS page (6): same fullscreen contract as page 5.
    else if (currentPage == 6) {
      if (physicalY >= 648) {
        handleFullscreenStripTouch(physicalX);

      } else if (raStatus.valid && raStatus.status == "ok" &&
                 raStatus.total > 0) {
        // --- Content band: subpage cycle / row open / detail close ---------
        ...
        // Row bands live in PHYSICAL panel space (Board.Touch is 1:1): row i is
        // centred at RA_ROW_TOUCH_TOP + i*RA_ROW_TOUCH_PITCH, so the first
        // band starts at rowBandTop. Taps above that are the title/indicator
        // area -> cycle subpages. Taps from rowBandTop down to the nav strip
        // map to a row, clamped to the last one so there is no dead zone.
        ...
        Serial.println("  -> RA content tap (no trophy data) — ignored");
        lastButtonPress = millis();
      }
    }
    ...
    // Check PREV button
      ...
      if (isFullscreenPage(currentPage)) backgroundLoaded = false;  // own chrome
      needsRedraw = true;
      lastPageChange = millis();
      lastButtonPress = millis();
    }
    ...
    // Check SCAN button — global refresh, including forced ROM details rescan
      ...
      if (scanInProgress) {
        ...
        Board.Display.fillRect(btnScan.x + 10, btnScan.y + 24,
...
//
// This acts on Board.Display rather than on the Lcd (ScaledDisplay) wrapper:
...
void applyDisplayFlip(bool flip) {
  if (!flip) return;
  const uint8_t r = Board.Display.getRotation();
  Board.Display.setRotation((r & 4) | ((r + 2) & 3));
...
void setup() {
  ...
  // The audio codec is not driven by this port; Board.Speaker is a no-op.
  Serial.println("=== SPEAKER ===  (skipped: not driven on this board)");

  // Panel orientation, depth and brightness are set by Board.begin().
  ...
  Serial.println("Mode: SDMMC 4-bit");
  
  int sdRetries = 0;
  while (!Board.beginSD() && sdRetries < 5) {
    sdRetries++;
...
// Logo centred horizontally with its top edge at yTop, at the given
// brightness (0-255), shared by every standby screen. Channels are scaled
// with an ordered dither: 5 and 6 bits per channel band visibly on a gradient
// once dimmed, and a fixed dither pattern hides it without shimmering between
// repaints.
static void drawStandbyLogo(int level, int yTop) {
  ...
  if (!standbyLogoPixels) {
    ...
    int c = level >> 3;
    Board.Display.setTextSize(6);                     // 36 px per character
    Board.Display.setTextColor(Board.Display.color565(c << 3, c << 3, c << 3), THEME_BLACK);
    Board.Display.setCursor((TARGET_WIDTH - (int)strlen(label) * 36) / 2, yTop);
  ...
  for (int y = 0; y < standbyLogoH; y++) {
    ...
    Board.Display.pushImage(x0, y0 + y, standbyLogoW, 1, rowBuf);
  }
  Board.Display.endWrite();
}
...
// corner. No time here: the clock screen is where the time belongs.
static void drawStandbyMinimal(bool fullRedraw) {
  ...
  int level = standbyBreathLevel(millis(), STANDBY_BREATH_MS);
  if (level == standbyDrawnLevel) return;
  drawStandbyLogo(standbyDimLevel(level), (TARGET_HEIGHT - standbyLogoHeight()) / 2);
  standbyDrawnLevel = level;
}

// ---- Clock screen: logo, seven-segment time, date, link status line ----
#define STANDBY_CLOCK_LOGO_Y    24
#define STANDBY_DIGIT_Y         240
#define STANDBY_DIGIT_W         112
#define STANDBY_DIGIT_H         200
#define STANDBY_DIGIT_T         24      // segment thickness
#define STANDBY_DIGIT_GAP       30      // between digits, and around the colon
#define STANDBY_COLON_W         40
#define STANDBY_SEG_JOIN        3       // dark gap where two segments meet
#define STANDBY_DATE_Y          476
#define STANDBY_RULE_Y          548
#define STANDBY_STATUS_TOP      584     // band shared by the status and game lines
#define STANDBY_STATUS_BAND_H   112
#define STANDBY_STATUS_Y        604     // status line when the game line is under it
#define STANDBY_STATUS_Y_ALONE  624     // status line on its own
#define STANDBY_GAME_Y          650
#define STANDBY_TEXT_MAX_W      (TARGET_WIDTH - 160)
#define STANDBY_SEG_LIT         THEME_CYAN
#define STANDBY_SEG_GHOST       0x08E3  // unlit segments: a faint tint of the lit
                                        // colour, meant to be barely there.
                                        // Raise it for a stronger LCD look.
...
static void drawStandbySegH(int x0, int x1, int yc, int t, uint16_t color) {
  int h = t / 2;
  Board.Display.fillRect(x0 + h, yc - h, (x1 - x0) - 2 * h + 1, 2 * h + 1, color);
  Board.Display.fillTriangle(x0, yc, x0 + h, yc - h, x0 + h, yc + h, color);
  Board.Display.fillTriangle(x1, yc, x1 - h, yc - h, x1 - h, yc + h, color);
}

static void drawStandbySegV(int xc, int y0, int y1, int t, uint16_t color) {
  int h = t / 2;
  Board.Display.fillRect(xc - h, y0 + h, 2 * h + 1, (y1 - y0) - 2 * h + 1, color);
  Board.Display.fillTriangle(xc, y0, xc - h, y0 + h, xc + h, y0 + h, color);
  Board.Display.fillTriangle(xc, y1, xc - h, y1 - h, xc + h, y1 - h, color);
}
...
// Text in a proportional font, centred on the panel. Text wider than
// STANDBY_TEXT_MAX_W is cut short and ends in "...". The caller clears the
// band first. Returns the x the text starts at. Leaves the default font
// selected: the rest of the interface assumes it.
static int drawStandbyCentredText(String text, const lgfx::IFont* font, int y, uint16_t color) {
  Board.Display.setFont(font);
  Board.Display.setTextSize(1);
  if ((int)Board.Display.textWidth(text.c_str()) > STANDBY_TEXT_MAX_W) {
    while (text.length() > 1 &&
           (int)Board.Display.textWidth((text + "...").c_str()) > STANDBY_TEXT_MAX_W) {
      text.remove(text.length() - 1);
    }
    text.trim();
    text += "...";
  }
  int x = (TARGET_WIDTH - (int)Board.Display.textWidth(text.c_str())) / 2;
  Board.Display.setTextColor(standbyDimColor(color), THEME_BLACK);
  Board.Display.setCursor(x, y);
  Board.Display.print(text);
  Board.Display.setFont(&fonts::Font0);
  return x;
}
...
static void drawStandbyClock(bool fullRedraw, const struct tm* t) {
  ...
  if (fullRedraw) {
    Board.Display.fillScreen(THEME_BLACK);
    standbyLoadLogo();
    drawStandbyLogo(standbyDimLevel(255), STANDBY_CLOCK_LOGO_Y);
    int cx = x0 + 2 * step + STANDBY_COLON_W / 2;
    int d  = STANDBY_DIGIT_T;
    uint16_t lit = standbyDimColor(STANDBY_SEG_LIT);
    Board.Display.fillRect(cx - d / 2, STANDBY_DIGIT_Y + STANDBY_DIGIT_H / 3 - d / 2, d, d, lit);
    Board.Display.fillRect(cx - d / 2, STANDBY_DIGIT_Y + 2 * STANDBY_DIGIT_H / 3 - d / 2, d, d, lit);
    Board.Display.fillRect(340, STANDBY_RULE_Y, 600, 2, standbyDimColor(0x4208));
  ...
  if (!CLOCK_24H) {
    ...
    if (pm != standbyDrawnAmPm) {
      Board.Display.setFont(&fonts::Orbitron_Light_24);
      Board.Display.setTextSize(1);
      Board.Display.setTextColor(standbyDimColor(0x0410), THEME_BLACK);
      Board.Display.setCursor(x0 + total + 20, STANDBY_DIGIT_Y + STANDBY_DIGIT_H - 26);
      Board.Display.print(pm ? "PM" : "AM");
      Board.Display.setFont(&fonts::Font0);
      standbyDrawnAmPm = pm;
    }
  }
  ...
  if (t->tm_yday != standbyDrawnDay) {
    ...
    Board.Display.fillRect(0, STANDBY_DATE_Y - 4, TARGET_WIDTH, 48, THEME_BLACK);
    drawStandbyCentredText(buf, &fonts::Orbitron_Light_32, STANDBY_DATE_Y, THEME_YELLOW);
  ...
  if (link != standbyDrawnLink) {
    bool showGame = connected && currentGame.length() > 0;
    String line   = connected ? String("MiSTer  ONLINE  -  ") + foldForDisplay(currentCore)
                              : String("MiSTer  OFFLINE");
    int y = showGame ? STANDBY_STATUS_Y : STANDBY_STATUS_Y_ALONE;
    Board.Display.fillRect(0, STANDBY_STATUS_TOP, TARGET_WIDTH, STANDBY_STATUS_BAND_H, THEME_BLACK);
    int x = drawStandbyCentredText(line, &fonts::Orbitron_Light_24, y, connected ? 0x05E0 : 0xBBE0);
    if (showGame) {
      drawStandbyCentredText(foldForDisplay(currentGame), &fonts::Orbitron_Light_24,
                             STANDBY_GAME_Y, 0xBDE0);
    }
    standbyStatusDotX = x - 24;
    standbyStatusDotY = y + 12;
    standbyDrawnLink  = link;
    standbyDrawnPulse = -1;
  }
  ...
  if (pulse != standbyDrawnPulse) {
    ...
    Board.Display.fillCircle(standbyStatusDotX, standbyStatusDotY, 8,
                          standbyDimColor(Board.Display.color565(r * pulse / 255, g * pulse / 255, 0)));
...
void loop() {
  ...
  // spent by returning: the touch edge is gone by the next pass.
  {
    ...
    if (standbyActive) {
      ...
      standbyLeave(wake);
      Board.Display.setBrightness(DISPLAY_BRIGHTNESS_NORMAL);
      ...
      if (standbyShouldEnter()) {
        standbyEnter();
        Board.Display.setBrightness(STANDBY_BRIGHTNESS);
  ...
  if (touch.wasPressed()) {
    ...
    Board.Display.fillRect(0, 0, 1280, 80, THEME_CYAN);
    Board.Display.setTextSize(4);
    Board.Display.setTextColor(THEME_BLACK);
    Board.Display.setCursor(400, 25);
    Board.Display.print("LOADING INTERFACE...");
    showingCoreImage         = false;
    backgroundLoaded         = false;
    needsRedraw              = true;
    lastButtonPress          = millis();
      ...
      if (currentGame.length() > 0 && g_imageFooterVisible) {
        ...
        if (millis() - lastFooterUpdate > 100) {
          if (imageFooterScroll.needsScroll && imageFooterScroll.fullText.length() > 0) {
            ...
            Board.Display.setTextColor(THEME_YELLOW, THEME_BLACK);
            Board.Display.setTextSize(3);
            Board.Display.setCursor(textCursorX, gameLineY);
            Board.Display.print(scrolledText);
          }
  ...
  if (!showingCoreImage && sdCardAvailable && timeToLeave
    ...
    if (shouldShowGame) {
      ...
      showGameSlide();   // every entry into image mode restarts the cycle on the game image
    } else {
...
if (oldGame != currentGame && sdCardAvailable) {
  ...
  if (millis() - lastFooterScrollUpdate > 100) { // Update every 100ms
    ...
    if (!showingCoreImage && !isFullscreenPage(currentPage) &&
      ...
      if (gameFooterScroll.fullText == textToShow) {
        ...
        Board.Display.setTextColor(THEME_CYAN, THEME_BLACK);
        Board.Display.setTextSize(2);
        Board.Display.setCursor(textStartX_phys, 220 * SCALE_Y);
        Board.Display.print(displayText);
      }
    }
  ...
  if (currentPage == 5 && !showingCoreImage &&
    ...
    if (gameInfoTitleScroll.needsScroll) {
      ...
      Board.Display.setTextColor(THEME_YELLOW, THEME_BLACK);
      Board.Display.setTextSize(4);
      Board.Display.setCursor(30, 16);
      Board.Display.print(displayTitle);
    }
  ...
  if (currentPage == 5 && !showingCoreImage && gameInfoSubPage == 0 &&
    ...
    Board.Display.setTextWrap(false);
    Board.Display.setTextColor(THEME_WHITE, THEME_BLACK);
    Board.Display.setTextSize(4);
    for (int r = 0; r < 6; r++) {
      ...
      Board.Display.setCursor(GI_VAL_X, gameInfoRowY[r]);
  ...
  // The title test matters: raUnlockScroll keeps the previous game's state, so
  // without it a stale needsScroll would animate a dead title straight over the
  // fallback line drawn at the very same cursor.
  if (currentPage == 6 && !showingCoreImage && raSubPage == 0 &&
      raStatus.valid && raStatus.status == "ok" &&
      raStatus.lastUnlockTitle.length() > 0 && raUnlockScroll.needsScroll &&
    ...
    Board.Display.setTextColor(THEME_YELLOW, THEME_BLACK);
    Board.Display.setTextSize(4);
    Board.Display.setCursor(40, 372);
    Board.Display.print(raShown);
...
void initSDCard() {
  ...
  // Mount the card through the HAL
  if (Board.beginSD()) {
    sdCardAvailable = true;
    Serial.println("SD card initialized successfully");
...
int jpegDrawCallback(JPEGDRAW *pDraw) {
  ...
  if (g_fineActive) {
    ...
    if (runLen <= FINE_BLOCK_MAX && rowCnt <= FINE_BLOCK_MAX) {
      ...
      Board.Display.pushImage(outX, outY, runLen, rowCnt, g_fineBlock);
      return 1;
    }
    ...
    for (int dy = dyFirst; dy <= dyLast; dy++) {
      ...
      Board.Display.pushImage(outX, dy + g_jpegOffsetY, runLen, 1, g_fineRow);
    }
    return 1;
  }
  ...
  if (finalX >= 0 && finalY >= 0 && 
    ...
    Board.Display.pushImage(finalX, finalY, pDraw->iWidth, pDraw->iHeight, pDraw->pPixels);
...
void showMenuImageWithCoreOverlay(String coreName) {
  ...
  if (coreName.equalsIgnoreCase("MENU")) {
    ...
    // Footer for the pure MENU state (no game, no core overlay). Painted
    // inline rather than through drawCoreImageFooter(), so it needs the
    // kiosk guard of its own.
  ...
  if (menuImageFound && displayCoreImage(menuImagePath)) {
    ...
    if (shouldShowError) {
      ...
      Board.Display.fillRect(1100, 20, 160, 40, THEME_BLACK);
      Board.Display.drawRect(1100, 20, 160, 40, THEME_RED);
      Board.Display.setTextColor(THEME_RED);
      Board.Display.setTextSize(2);
      Board.Display.setCursor(1115, 32);
      
      if (displayError == "NO SERVER") {
        Board.Display.print("NO SERVER");
      } else if (displayError == "TIMEOUT") {
        Board.Display.print("TIMEOUT");
      } else if (displayError == "OFFLINE") {
        Board.Display.print("OFFLINE");
      } else if (displayError == "DISCONNECTED") {
        Board.Display.print("DISCONN.");
      } else if (displayError.startsWith("ERROR")) {
        Board.Display.print("ERROR");
      } else {
        Board.Display.print(displayError.substring(0, 10));
      }
  ...
  Board.Display.setTextColor(THEME_GREEN);
  Board.Display.setTextSize(2);
  Board.Display.setCursor(20, 632);   // size 2 label, slightly above the size-3 value baseline
  Board.Display.print("ACTIVE CORE:");
  
  Board.Display.setTextColor(THEME_YELLOW);
  Board.Display.setTextSize(3);
  Board.Display.setCursor(245, 628);
  String coreFold = foldForDisplay(coreName);   // display-only fold
  String footerCore = coreFold.length() > 30 ? coreFold.substring(0, 30) : coreFold;
  if (footerCore.equalsIgnoreCase("arcade")) footerCore = "Arcade";
  Board.Display.print(footerCore);
  ...
  if (hasGame) {
    ...
    Board.Display.setTextColor(THEME_CYAN);
    Board.Display.setTextSize(2);
    Board.Display.setCursor(20, 672);
    Board.Display.print("GAME:");
    
     Board.Display.setTextColor(THEME_YELLOW, THEME_BLACK);
    Board.Display.setTextSize(3);
    Board.Display.setCursor(120, 668);
    Board.Display.print(displayGame);
  }
  ...
  // game at all that row is empty, so it is centred instead (x=451).
    ...
    if (showInfoButton) {
      Board.Display.setTextSize(2);
      Board.Display.setCursor(600, 695);
    } else {
      Board.Display.setTextSize(3);
      Board.Display.setCursor(hasGame ? 882 : 451, 668);
    }
    Board.Display.print("Touch to show monitor");
...
void showCoreNotFoundScreen(String coreName) {
  ...
  Board.Display.setTextSize(3);
  if (!imageFooterSuppressed()) {
    Board.Display.setCursor(350, 660);
    Board.Display.print("Press any button for interface");
    noteImageFooterShown();
  }
  }
...
bool loadFullScreenFrame(const char* framePath) {
  ...
  auto frameCallback = [](JPEGDRAW *pDraw) -> int {
    Board.Display.pushImage(pDraw->x, pDraw->y, pDraw->iWidth, pDraw->iHeight, pDraw->pPixels);
    return 1;
  };
...
void drawCyberpunkFrame() {
  ...
  for (int t = 0; t < frameThick; t++) {
    // Top edge
    Board.Display.drawLine(left + cornerCut, top + t, right - cornerCut, top + t, THEME_CYAN);
    // Bottom edge
    Board.Display.drawLine(left + cornerCut, bottom - t, right - cornerCut, bottom - t, THEME_CYAN);
    // Left edge
    Board.Display.drawLine(left + t, top + cornerCut, left + t, bottom - cornerCut, THEME_CYAN);
    // Right edge
    Board.Display.drawLine(right - t, top + cornerCut, right - t, bottom - cornerCut, THEME_CYAN);
  }
  
  // Corner diagonal cuts
  for (int t = 0; t < frameThick; t++) {
    Board.Display.drawLine(left + t, top + cornerCut, left + cornerCut, top + t, THEME_CYAN);
    Board.Display.drawLine(right - cornerCut, top + t, right - t, top + cornerCut, THEME_CYAN);
    Board.Display.drawLine(left + t, bottom - cornerCut, left + cornerCut, bottom - t, THEME_CYAN);
    Board.Display.drawLine(right - cornerCut, bottom - t, right - t, bottom - cornerCut, THEME_CYAN);
  }
  
  // Center divider line
  int dividerX = 640;
  for (int t = 0; t < 2; t++) {
    Board.Display.drawFastVLine(dividerX + t, top + 50, bottom - top - 100, THEME_CYAN);
  }
...
void drawProgressSquares(int completedCount) {
  ...
  for (int i = 0; i < 5; i++) {
    ...
    // Filled square
    Board.Display.fillRect(x, startY, squareSize, squareSize, color);
    
    // Border
    Board.Display.drawRect(x - 2, startY - 2, squareSize + 4, squareSize + 4, THEME_CYAN);
    
    // Inner detail
    if (i < completedCount) {
      // Checkmark pattern for completed
      Board.Display.drawLine(x + 15, startY + 30, x + 25, startY + 40, THEME_WHITE);
      Board.Display.drawLine(x + 25, startY + 40, x + 45, startY + 15, THEME_WHITE);
    }
  }
}
...
void showBootSequence() {
  ...
  Board.Display.setTextColor(THEME_GREEN);
  Board.Display.setTextSize(4);
  Board.Display.setCursor(bootOffsetX + 80*scale, bootOffsetY + 20*scale);
  Board.Display.print("SYSTEM");
  
  Board.Display.setTextColor(THEME_YELLOW);
  Board.Display.setCursor(bootOffsetX + 40*scale, bootOffsetY + 45*scale);
  Board.Display.print("INITIALIZE");
  ...
  for (int i = 0; i < 5; i++) {
    ...
    Board.Display.setCursor(bootOffsetX, lineY);
    
    // Typing effect
    for (int j = 0; j < bootLines[i].length(); j++) {
      Board.Display.print(bootLines[i].charAt(j));
      delay(20);
    }
  ...
  Board.Display.setTextSize(2);
  Board.Display.setTextColor(THEME_WHITE);
  Board.Display.setCursor(bootOffsetX, bootOffsetY + 196*scale);
  Board.Display.print("FW ");
  Board.Display.print(FIRMWARE_VERSION);
  ...
  Board.Display.drawRect(barX, barY, barW, barH, THEME_WHITE);
  Board.Display.fillRect(barX + scale, barY + scale, barW - 2*scale, barH - 2*scale, THEME_BLACK);
...
void drawWiFiProgressCircles(int currentAttempt, bool connected, int maxAttempts = 30) {
  ...
  Board.Display.fillRect(780, 310, 400, 80, THEME_BLACK);
  ...
  for (int i = 0; i < circlesToDraw; i++) {
    ...
    // Draw filled circle
    Board.Display.fillCircle(x, y, circleRadius, fillColor);
    
    // Draw border
    Board.Display.drawCircle(x, y, circleRadius + 1, borderColor);
    
    // Add inner highlight for 3D effect
    if (connected) {
      Board.Display.fillCircle(x - 3, y - 3, 3, THEME_WHITE);
    }
  }
  ...
  Board.Display.setTextColor(THEME_CYAN);
  Board.Display.setTextSize(2);
  Board.Display.setCursor(startX, startY + 100);
  
  if (connected) {
    Board.Display.setTextColor(THEME_GREEN);
    Board.Display.setTextSize(3);
    Board.Display.print("CONNECTED");
  } else {
    Board.Display.printf("ATTEMPT %02d/%02d", currentAttempt, maxAttempts);
  }
}
...
void connectWithAnimation() {
  ...
  Board.Display.fillRect(90, 180, 640, 410, THEME_BLACK);  // Clear left panel only
  ...
  Board.Display.setTextColor(THEME_CYAN);
  Board.Display.setTextSize(4); // 2x original size
  Board.Display.setCursor(animOffsetX + 60*scale, animOffsetY + 15*scale);
  Board.Display.print("CONNECTING");
  ...
  // WiFi runs on a coprocessor over SDIO. The link is brought up once and
  // polled: a scan before connecting, or repeated disconnect()/begin()
  // cycles, leave the radio unable to associate and leak the SDIO link's
  // memory pool. setAutoReconnect() makes the driver retry the association
  ...
  while (WiFi.status() != WL_CONNECTED && attempts < maxAttempts) {
    ...
    Board.Display.fillRect(animOffsetX, animOffsetY + 60*scale, 
    ...
    for (int r = 20*scale; r <= radarRadius; r += 20*scale) {
      Board.Display.drawCircle(radarCenterX, radarCenterY, r, THEME_GREEN);
    }
    ...
    Board.Display.drawLine(radarCenterX, radarCenterY, endX, endY, THEME_YELLOW);
    
    // Draw radar crosshair
    Board.Display.drawFastHLine(radarCenterX - 10*scale, radarCenterY, 20*scale, THEME_CYAN);
    Board.Display.drawFastVLine(radarCenterX, radarCenterY - 10*scale, 20*scale, THEME_CYAN);
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    // Board.Display.setTextColor(THEME_GREEN);
    // Board.Display.setTextSize(4);
    // Board.Display.setCursor(animOffsetX + 40*scale, animOffsetY + 95*scale);
    // Board.Display.print("CONNECTED");
    
    Board.Display.setTextColor(THEME_CYAN);
    Board.Display.setTextSize(2);
    Board.Display.setCursor(animOffsetX + 30*scale, animOffsetY + 230*scale);
    Board.Display.printf("IP: %s", WiFi.localIP().toString().c_str());
    ...
    Board.Display.setTextColor(THEME_YELLOW);
    Board.Display.setTextSize(2);
    Board.Display.setCursor(animOffsetX + 45*scale, animOffsetY + 95*scale);
    Board.Display.print("Testing MiSTer...");
    ...
    Board.Display.setTextColor(THEME_RED);
    Board.Display.setTextSize(3);
    Board.Display.setCursor(animOffsetX + 35*scale, animOffsetY + 95*scale);
    Board.Display.print("WIFI FAILED");
    
    Serial.printf("Error connecting WiFi! (status %d)\n", (int)WiFi.status());

    // Diagnostic scan, only once the connection has already failed.
    ...
    Board.Display.fillRect(670, 320, 580, 120, THEME_BLACK);
    ...
    for (int i = 0; i < attempts; i++) {
      ...
      int x = 670 + col * 39;
      int y = 320 + row * 39;
      Board.Display.fillCircle(x, y, 12, THEME_RED);
      Board.Display.drawCircle(x, y, 13, THEME_YELLOW);
    }
    
    Board.Display.setTextColor(THEME_RED);
    Board.Display.setTextSize(2);
    Board.Display.setCursor(670, 420);
    Board.Display.print("CONNECTION FAILED");
  }
...
void testMiSTerConnectivity(bool discovered) {
  ...
  if (strlen(misterIP) == 0) {
    ...
    Board.Display.fillRect(animOffsetX, animOffsetY + 60*scale, 520, 140*scale, THEME_BLACK);
    Board.Display.setTextColor(THEME_RED);
    Board.Display.setTextSize(3);
    Board.Display.setCursor(animOffsetX + 30*scale, animOffsetY + 95*scale);
    Board.Display.print("MiSTer NOT FOUND");
    Board.Display.setTextColor(THEME_YELLOW);
    Board.Display.setTextSize(2);
    Board.Display.setCursor(animOffsetX + 30*scale, animOffsetY + 120*scale);
    Board.Display.print("Set ip= in config.ini");
    connected = false;
    delay(2000);
    return;
  }
  ...
  Board.Display.fillRect(animOffsetX, animOffsetY + 60*scale, safeWidth, 140*scale, THEME_BLACK);
  ...
  if (code == 200) {
    ...
    Board.Display.setTextSize(3);
    Board.Display.setCursor(animOffsetX + 30*scale, animOffsetY + 95*scale);
    Board.Display.print("MiSTer: ONLINE");
    Board.Display.setCursor(animOffsetX + 30*scale, animOffsetY + 115*scale);
    Board.Display.printf("Server: %s:8081", misterIP);
    connected = true;
  } else {
    connected = false;
    ...
    if (discovered) {
      ...
      Board.Display.setCursor(animOffsetX + 30*scale, animOffsetY + 95*scale);
      Board.Display.print("MiSTer FOUND, no reply");
      Board.Display.setTextColor(THEME_YELLOW);
      Board.Display.setTextSize(2);
      Board.Display.setCursor(animOffsetX + 30*scale, animOffsetY + 120*scale);
      Board.Display.print("Is the script running?");
    } else {
      // Using the config.ini fallback IP and it doesn't answer → likely wrong IP.
      Board.Display.setCursor(animOffsetX + 30*scale, animOffsetY + 95*scale);
      Board.Display.print("MiSTer: OFFLINE");
      Board.Display.setCursor(animOffsetX + 30*scale, animOffsetY + 115*scale);
      Board.Display.printf("Check IP: %s", misterIP);
    }
  }
...
void showReconnectBanner() {
  ...
  Board.Display.fillRect(0, 620, 1280, 100, THEME_GREEN);
  Board.Display.setTextSize(3);
  Board.Display.setTextColor(THEME_BLACK, THEME_GREEN);
  Board.Display.setCursor((1280 - tw) / 2, 658);     // centered in the band
  Board.Display.print(msg);
...
void showRAReadyBanner() {
  // Same footer band (Y=620..720) and hold-then-restore shape as
  // showReconnectBanner(). "READY FOR RETROACHIEVEMENTS!" is 28 chars =
  // 504 px at size 3, so it fits centred on one line.
  ...
  Board.Display.fillRect(0, 620, 1280, 100, THEME_YELLOW);
  Board.Display.setTextWrap(false);
  Board.Display.setTextSize(3);
  Board.Display.setTextColor(THEME_BLUE, THEME_YELLOW);
  Board.Display.setCursor((1280 - tw) / 2, 658);     // centered in the band
  Board.Display.print(msg);

  delay(3400);                                    // hold long enough to read

  // Restore what the banner covered. Over a fullscreen image that is the
  // image footer; in page mode the band belongs to frame02.jpg plus
...
void showVersionMismatchBanner() {
  ...
  Board.Display.fillRect(0, 620, 1280, 100, THEME_RED);
  Board.Display.setTextWrap(false);
  Board.Display.setTextSize(3);
  Board.Display.setTextColor(THEME_WHITE, THEME_RED);
  Board.Display.setCursor((1280 - w1) / 2, 640);
  Board.Display.print(l1);
  Board.Display.setCursor((1280 - w2) / 2, 676);
  Board.Display.print(line2);
...
void getRAStatus() {
  ...
  // Same sentinel handling as last_unlock_description and the trophy-list
  // fields below: the server writes "N/A" for a string it cannot fill, and
  // this was the one field that took it literally and would have shown "N/A"
  // as the achievement name.
  if (raStatus.lastUnlockTitle == "N/A") raStatus.lastUnlockTitle = "";
  ...
  Serial.printf("[RA] status=%s matched=%d %d/%d pts=%d/%d hc=%d evt=%d last='%s'\n",
...
void drawRAMessage(const String &title, const String &subtitle, uint16_t color) {
  Board.Display.setTextWrap(false);
  Board.Display.setTextColor(color, THEME_BLACK);
  Board.Display.setTextSize(4);
  Board.Display.setCursor(60, 150);
  Board.Display.print(title);

  Board.Display.setTextColor(THEME_WHITE, THEME_BLACK);
  Board.Display.setTextSize(3);
  Board.Display.setCursor(60, 214);
  Board.Display.print(subtitle);
}

// Shared title row for the page-6 renderers: game title (yellow, size 4,
// capped at 40 chars so it never reaches the subpage indicator, which
// right-aligns to x=1250) plus the rule underneath. Content starts below y=70.
static void drawRATitleRow() {
  Board.Display.setTextWrap(false);
  Board.Display.setTextColor(THEME_YELLOW, THEME_BLACK);
  Board.Display.setTextSize(4);
  Board.Display.setCursor(30, 16);
  {
    ...
    Board.Display.print(t);
  }
  drawRAPageIndicator(false);
  Board.Display.drawFastHLine(30, 56, 1220, THEME_CYAN);
  Board.Display.drawFastHLine(30, 57, 1220, THEME_CYAN);
}

// Page 6 renderer, FULLSCREEN: the RA section owns the whole 1280x720 panel
// (bottom nav strip aside) instead of the 640x480 scaled window the other
// pages use. Clears the content region (the strip chrome persists) and
// branches on the endpoint's status field. unlocked==0 with status "ok" is a
// legitimate "not started yet" state, not an error.
void displayRetroAchievements() {
  Board.Display.fillRect(0, 0, 1280, 646, THEME_BLACK);
  ...
  if (raStatus.status == "not_configured") {
    ...
    Board.Display.setTextColor(THEME_YELLOW, THEME_BLACK);
    Board.Display.setTextSize(4);
    Board.Display.setCursor(60, 90);
    Board.Display.print("RA NOT SET UP");

    Board.Display.setTextColor(THEME_CYAN, THEME_BLACK);
    Board.Display.setTextSize(3);
    Board.Display.setCursor(60, 170);  Board.Display.print("To enable RetroAchievements:");
    Board.Display.setTextColor(THEME_WHITE, THEME_BLACK);
    Board.Display.setCursor(60, 230);  Board.Display.print("1. Get a Web API key at retroachievements.org");
    Board.Display.setCursor(60, 280);  Board.Display.print("2. On the MiSTer edit ra_credentials.ini");
    Board.Display.setCursor(60, 330);  Board.Display.print("3. Add username + api_key");
    Board.Display.setCursor(60, 380);  Board.Display.print("4. Restart the monitor server");
    return;
  }
  ...
  if (raStatus.status == "rom_not_recognized") {
    ...
    Board.Display.setTextColor(THEME_YELLOW, THEME_BLACK);
    Board.Display.setTextSize(4);
    Board.Display.setCursor(60, 100);
    Board.Display.print("GAME NOT IDENTIFIED");
    Board.Display.setTextColor(THEME_CYAN, THEME_BLACK);
    Board.Display.setTextSize(3);
    Board.Display.setCursor(60, 180);  Board.Display.print("RetroAchievements could not identify this game");
    Board.Display.setCursor(60, 220);  Board.Display.print("from its ROM or disc contents.");
    Board.Display.setTextColor(THEME_WHITE, THEME_BLACK);
    Board.Display.setCursor(60, 290);  Board.Display.print("Either no set exists for it, or this dump is not linked.");
    return;
  }
  ...
  Board.Display.setTextColor(THEME_CYAN, THEME_BLACK);
  Board.Display.setTextSize(3);
  Board.Display.setCursor(40, 86);
  Board.Display.printf("PROGRESS  %d/%d", raStatus.unlocked, raStatus.total);
  {
    String pctStr = String((int)pct) + "%";
    Board.Display.setTextColor(THEME_WHITE, THEME_BLACK);
    Board.Display.setTextSize(4);
    Board.Display.setCursor(1240 - (int)pctStr.length() * 24, 80);
    Board.Display.print(pctStr);
  }
  {
    // Bar, double white border, tiered fill like showDownloadProgress().
    Board.Display.drawRect(40, 126, 1200, 44, THEME_WHITE);
    Board.Display.drawRect(41, 127, 1198, 42, THEME_WHITE);
    Board.Display.fillRect(43, 129, 1194, 38, THEME_BLACK);
    int fillW = (int)(pct * 11.90f);           // 100% -> 1190 px
    if (fillW > 0) {
      uint16_t barColor = (pct < 30) ? THEME_YELLOW :
                          (pct < 70) ? THEME_CYAN   : THEME_GREEN;
      Board.Display.fillRect(45, 131, fillW, 34, barColor);
    }
  }

  // Stat panels row: POINTS + (HARDCORE | MATCHED)
  char pointsVal[24];
  snprintf(pointsVal, sizeof(pointsVal), "%d/%d",
           raStatus.pointsEarned, raStatus.pointsTotal);
  drawRAStatPanel(40, 200, 580, 104, "POINTS", String(pointsVal), THEME_GREEN);
  ...
  if (raStatus.forkLoadFailed || raStatus.forkGameMismatch) {
    ...
    drawRAStatPanel(660, 200, 580, 104, "UNLOCKS", "VIEW ONLY", THEME_ORANGE);
  } else if (!raCoreRecordsUnlocks()) {
    drawRAStatPanel(660, 200, 580, 104, "UNLOCKS", "NOT RECORDED", THEME_YELLOW);
  } else if (raStatus.unlockedHardcore > 0) {
    char hcVal[24];
    snprintf(hcVal, sizeof(hcVal), "%d (%dpt)",
             raStatus.unlockedHardcore, raStatus.pointsHardcore);
    drawRAStatPanel(660, 200, 580, 104, "HARDCORE", String(hcVal), THEME_RED);
  } else {
    String mm = (raStatus.matchMethod == "lastgame") ? "LIVE (fork)" : "BY HASH";
    drawRAStatPanel(660, 200, 580, 104, "MATCHED", mm, THEME_CYAN);
  }

  // Last unlocked achievement. The whole block sits one type step above where
  // it started: this is the panel's only prose, the band below y=336 is
  // otherwise empty, and it is what a player leans in to read after an unlock.
  //
  // The label is drawn UNCONDITIONALLY. The previous shape had two branches —
  // a title, or unlocked == 0 — and a game with unlocks whose latest one the
  // server does not name fell through both and painted nothing at all, so the
  // section simply disappeared with no way to tell a missing field from a
  // missing feature. Every state now says something.
  Board.Display.setTextColor(THEME_CYAN, THEME_BLACK);
  Board.Display.setTextSize(3);
  Board.Display.setCursor(40, 336);
  Board.Display.print("LAST UNLOCK");

  if (raStatus.lastUnlockTitle.length() > 0) {
    const int raVisibleChars = 50;   // x=40 + 50*24 = 1240, inside the panel
    ...
    Board.Display.setTextSize(4);
    Board.Display.setCursor(40, 372);
    Board.Display.print(shown);

    // The log tailer also carries the description: show it under the title.
    // Three lines at 67 chars hold 201, comfortably over the server's cap.
    if (raLastUnlockDesc.length() > 0) {
      drawWrappedTextPhys(raLastUnlockDesc, 40, 418, 67, 3, THEME_CYAN, 3);
    }
  } else {
    // No title from the server. Two very different reasons, so say which:
    // an untouched set, versus a set with progress the endpoint did not
    // attribute to a specific achievement.
    Board.Display.setTextColor(THEME_GRAY, THEME_BLACK);
    Board.Display.setTextSize(4);
    Board.Display.setCursor(40, 372);
    Board.Display.print(raStatus.unlocked == 0 ? "No achievements earned yet"
                                            : "No recent unlock reported");
  }
  ...
  if (!raCoreRecordsUnlocks() && raStatus.total > 0) {
    Board.Display.setTextColor(THEME_YELLOW, THEME_BLACK);
    Board.Display.setTextSize(3);
    Board.Display.setCursor(40, 530);
    Board.Display.print("Use an RA-enabled core to record unlocks");
  }
}

// Unlock popup overlay, fullscreen-proportional: a 900x420 card centred on
// the 1280x720 panel, drawn directly on Board.Display so it reads the same over
// a monitor page AND over the fullscreen artwork (the old card lived in the
// scaled 640x480 window and looked like a corner toast over images).
// loop() holds it 5 s; serviceRAPopup() restores underneath — page mode
// forces a full background reload because the card overhangs the frame art.
void showAchievementUnlock() {
  const int bx = 190, by = 150, bw = 900, bh = 420;
  const int tx = bx + 24;

  Board.Display.fillRect(bx, by, bw, bh, THEME_BLACK);
  Board.Display.drawRect(bx, by, bw, bh, THEME_YELLOW);
  Board.Display.drawRect(bx + 1, by + 1, bw - 2, bh - 2, THEME_YELLOW);
  Board.Display.drawRect(bx + 2, by + 2, bw - 4, bh - 4, THEME_YELLOW);

  Board.Display.setTextWrap(false);
  Board.Display.setTextColor(THEME_YELLOW, THEME_BLACK);
  Board.Display.setTextSize(4);
  Board.Display.setCursor(tx, by + 22);
  Board.Display.print("ACHIEVEMENT!");

  // Title: up to 2 wrapped lines at size 4 ((900-48)/24 = 35 chars/line).
  int y = drawWrappedTextPhys(raStatus.lastUnlockTitle, tx, by + 78, 35, 2,
                              THEME_WHITE, 4);
  ...
  if (raLastUnlockDesc.length() > 0) {
    y = drawWrappedTextPhys(raLastUnlockDesc, tx, y + 10, 47, 3, THEME_CYAN, 3);
  }
  ...
  Board.Display.setTextWrap(false);
  Board.Display.setTextSize(3);
  Board.Display.setTextColor(raStatus.lastUnlockHardcore ? THEME_RED : THEME_GREEN,
                          THEME_BLACK);
  Board.Display.setCursor(tx, by + bh - 44);
  if (raStatus.lastUnlockPoints > 0) {
    Board.Display.printf("+%d pts%s", raStatus.lastUnlockPoints,
                      raStatus.lastUnlockHardcore ? "  [HARDCORE]" : "");
  } else {
    Board.Display.print(raStatus.lastUnlockHardcore ? "UNLOCKED  [HARDCORE]" : "UNLOCKED");
  }
...
void serviceRAPopup() {
  ...
  if (millis() < raPopupUntil) {
    ...
    if (showingCoreImage) {
      ...
      backgroundLoaded = false;   // card overhangs the frame art / chrome
      needsRedraw = true;
    }
  }
}
...
// Physical-space sibling of drawWrappedText(): same wrap algorithm, but on
// Board.Display at an arbitrary text size. Line height = glyph height + 6 px of
// leading. Returns the y just below the last line drawn.
int drawWrappedTextPhys(const String& text, int x, int y, int charsPerLine,
                        int maxLines, uint16_t color, int textSize) {
  Board.Display.setTextWrap(false);
  Board.Display.setTextColor(color, THEME_BLACK);
  Board.Display.setTextSize(textSize);
  const int lineH = textSize * 8 + 6;
  String rest = text;
  rest.trim();
  int lines = 0;
...
// Bordered stat box for the progress panel (POINTS / HARDCORE / MATCHED):
// label small on top, value big underneath. Physical-space counterpart of the
// logical drawMiniPanel() the other pages use.
void drawRAStatPanel(int x, int y, int w, int h, const char* label,
                     const String& value, uint16_t color) {
  Board.Display.drawRect(x, y, w, h, color);
  Board.Display.drawRect(x + 1, y + 1, w - 2, h - 2, color);
  Board.Display.fillRect(x + 2, y + 2, w - 4, h - 4, THEME_BLACK);

  Board.Display.setTextWrap(false);
  Board.Display.setTextColor(color, THEME_BLACK);
  Board.Display.setTextSize(2);
  Board.Display.setCursor(x + 16, y + 14);
  Board.Display.print(label);

  Board.Display.setTextColor(THEME_WHITE, THEME_BLACK);
  Board.Display.setTextSize(4);
  Board.Display.setCursor(x + 16, y + 50);
  Board.Display.print(value);
}

// Pages 5 (GAME INFO) and 6 (RETROACHIEVEMENTS) own the whole panel instead
// of drawing inside the monitor window of frame02.jpg. Both share the bottom
// nav strip, so several call sites need to ask about the pair rather than
// about one page.
bool isFullscreenPage(int p) { return p == 5 || p == 6; }

// The three nav labels of the fullscreen bottom strip. pressedZone flashes one
// white (0=PREV, 1=SCAN, 2=NEXT; -1 = none) — the strip's counterpart of
// buttonPressFeedback()'s white-label flash.
void drawFullscreenNavHints(int pressedZone) {
  struct { const char* label; int x; } z[3] = {
    { "< PREV", 159 }, { "SCAN", 604 }, { "NEXT >", 1012 } };
  Board.Display.setTextWrap(false);
  Board.Display.setTextSize(3);
  for (int i = 0; i < 3; i++) {
    Board.Display.setTextColor(i == pressedZone ? THEME_WHITE : THEME_GREEN,
                            THEME_BLACK);
    Board.Display.setCursor(z[i].x, 664);
    Board.Display.print(z[i].label);
  }
}

// Static chrome shared by the fullscreen pages: separator rule, the CYD28-style
// three-thirds nav strip (tap zones with text hints instead of the frame02
// button column), and the 7 page dots. Drawn once per page entry via the
// backgroundLoaded flag; the renderers only ever clear y < 646.
void drawFullscreenChrome() {
  Board.Display.drawFastHLine(0, 646, 1280, THEME_CYAN);
  Board.Display.drawFastHLine(0, 647, 1280, THEME_CYAN);
  Board.Display.drawFastVLine(427, 656, 56, THEME_GRAY);
  Board.Display.drawFastVLine(853, 656, 56, THEME_GRAY);
  drawFullscreenNavHints(-1);

  // Page dots, centred in the SCAN zone under its label. Highlights whichever
  // fullscreen page is up, so the strip reads the same as the frame02 pages.
  for (int i = 0; i < totalPages; i++) {
    int dx = 561 + i * 24;
    if (i == currentPage) {
      Board.Display.fillRect(dx, 696, 14, 14, THEME_YELLOW);
      Board.Display.drawRect(dx - 2, 694, 18, 18, THEME_YELLOW);
    } else {
      Board.Display.fillRect(dx, 696, 14, 14, THEME_GRAY);
    }
  }
}

// Description overlay for the achievement at raDetailRow: title, full
// description, points and state, on a centred fullscreen card. Dismissed by
// any content tap (handleTouch clears raDetailShown). Guards the row index
// in case the buffer changed under it.
void displayRADetail() {
  Board.Display.fillRect(0, 0, 1280, 646, THEME_BLACK);
  const int bx = 140, by = 100, bw = 1000, bh = 460;
  Board.Display.drawRect(bx, by, bw, bh, THEME_YELLOW);
  Board.Display.drawRect(bx + 1, by + 1, bw - 2, bh - 2, THEME_YELLOW);
  ...
  // The card is the one place with room to spare, so everything in it runs a
  // size above the list it was opened from: this is the reading view.
  const int tx = bx + 24;
  int y = by + 20;

  // Title (up to 2 lines, (1000-48)/30 = 31 chars per line at size 5)
  y = drawWrappedTextPhys(raListTitle[i], tx, y, 31, 2, THEME_YELLOW, 5);
  y += 6;

  // Points + state
  Board.Display.setTextWrap(false);
  Board.Display.setTextColor(stateColor, THEME_BLACK);
  Board.Display.setTextSize(4);
  Board.Display.setCursor(tx, y);
  Board.Display.printf("%d pts   %s", raListPoints[i], stateLabel);
  y += 46;

  // Separator rule
  Board.Display.drawFastHLine(tx, y, bw - 48, THEME_GRAY);
  y += 16;

  // Description (up to 5 lines, (1000-48)/24 = 39 chars per line at size 4).
  // 5 x 39 = 195 chars, well over the server's description cap, and the block
  // ends at y=470 — clear of the close hint at y=520.
  String d = raListDesc[i];
  d.trim();
  if (d.length() == 0) d = "(No description provided.)";
  drawWrappedTextPhys(d, tx, y, 39, 5, THEME_CYAN, 4);

  // Close hint, bottom-right inside the card
  Board.Display.setTextColor(THEME_GRAY, THEME_BLACK);
  Board.Display.setTextSize(3);
  Board.Display.setCursor(bx + bw - 232, by + bh - 40);
  Board.Display.print("tap to close");
}
...
void drawRAPageIndicator(bool pressed) {
  ...
  int x = 1250 - (int)ind.length() * 18;   // right-aligned, size 3 = 18 px/char
  Board.Display.setTextWrap(false);
  Board.Display.setTextColor(pressed ? THEME_WHITE : THEME_CYAN, THEME_BLACK);
  Board.Display.setTextSize(3);
  Board.Display.setCursor(x, 24);
  Board.Display.print(ind);
}

// Page-6 subpage renderer, FULLSCREEN: one page of the trophy list. Row:
//   [H] title (one line, size 4) ............. 25p    hardcore (red mark)
//   [*] title ................................ 10p    softcore (green mark)
//   [ ] title .................................  5p    locked (gray, dim)
//       one-line description teaser (gray, size 3), under its own title
//
// TYPE SCALE vs ROW COUNT: at size 4 a title is 38 px per line and the teaser
// 24 px, so a two-line title would need 106 px of the 100 px pitch. The pitch
// is what buys the 145 px header tap target, and dropping to four rows per
// page would lengthen every browse. The title therefore gets one 40-char line
// and ellipsises; the full text is one tap away in the detail card.
// Rows y = 145..545 step 100 (5 per page); the nav strip (y>=646) stays
// untouched. Row hit bands live in RA_ROW_TOUCH_TOP/PITCH, derived from
// these draw positions — keep the two in step if this layout ever changes.
//
// The rows start at 145 rather than hard under the title rule: everything
// above the first row band is the subpage-cycle tap target, and 145 px makes
// that a comfortable finger target instead of a sliver.
void displayRAList() {
  Board.Display.fillRect(0, 0, 1280, 646, THEME_BLACK);
  ...
  for (int i = 0; i < raListCount; i++) {
    ...
    // Mark, title and points are all size 4, so they share the same +2
    // offset: the mark used to sit at +10 and read as belonging to the line
    // below its own title.
    Board.Display.setTextWrap(false);
    Board.Display.setTextColor(markColor, THEME_BLACK);
    Board.Display.setTextSize(4);
    Board.Display.setCursor(40, rowY + 2);
    Board.Display.print(mark);

    // Title: one line of 40 chars at size 4 (x 130..1090, clear of the points
    // column at x=1150). The column moved right from 120 to clear the now
    // wider mark. The return value is the y just below the line actually
    // drawn, which is what the teaser hangs off.
    int titleBottom = drawWrappedTextPhys(raListTitle[i], 130, rowY + 2, 40, 1,
                        raListUnlocked[i] ? THEME_WHITE : THEME_GRAY, 4);

    Board.Display.setTextColor(raListUnlocked[i] ? THEME_CYAN : THEME_GRAY,
                            THEME_BLACK);
    Board.Display.setTextSize(4);
    Board.Display.setCursor(1150, rowY + 2);
    Board.Display.printf("%3dp", raListPoints[i]);

    // One-line description teaser, hung off the title's real bottom rather
    // than a fixed row offset: the old +70 was measured for a two-line title,
    // so the far more common one-line title left the description stranded 44 px
    // below the words it belongs to.
    String d = raListDesc[i];
    d.trim();
    if (d.length() > 0) {
      if (d.length() > 62) d = d.substring(0, 59) + "...";   // 62*18 = 1116 px
      Board.Display.setTextColor(THEME_GRAY, THEME_BLACK);
      Board.Display.setTextSize(3);
      Board.Display.setCursor(130, titleBottom + 4);
      Board.Display.print(d);
    }
  }
}
...
void updateDisplay() {
  ...
  switch(currentPage) {
    ...
    case 5:
      // GAME INFO page — FULLSCREEN, same contract as page 6 below: no
      // frame02.jpg, the whole panel plus the shared bottom nav strip.
      if (!backgroundLoaded) {
        Board.Display.fillScreen(THEME_BLACK);
        drawFullscreenChrome();
        backgroundLoaded = true;
      }
      displayGameInfo();
      break;
    case 6:
      // RETROACHIEVEMENTS page — FULLSCREEN. Like page 5 and unlike pages 0-4
      // it does not load frame02.jpg: it owns the whole panel, with a slim
      // CYD28-style nav strip instead of the button column. backgroundLoaded
      // keeps its usual meaning ("static chrome is on screen"), so every
      // existing entry path (image-mode exits, banner/popup restores, the
      // generic PRV/NXT handlers) repaints it for free.
      if (!backgroundLoaded) {
        Board.Display.fillScreen(THEME_BLACK);
        drawFullscreenChrome();
        backgroundLoaded = true;
      }
      if (raSubPage == 0)      displayRetroAchievements();
...
void displayNetworkTerminal() {
  ...
  // ========== ORIGINAL 320x240 CONTENT (auto-scaled 2x by Lcd) ==========
  // Main panel - based on display ↔ MiSTer connection
...
// squares (drawPageIndicators) live at physical X 410..692 / Y 210..240, past
...
void drawHudConnectionDot(bool isConnected, bool active) {
  ...
  if (active) {
    Board.Display.fillCircle(cx, cy, r, isConnected ? THEME_GREEN : THEME_RED);
    Board.Display.fillCircle(cx, cy, r / 2, THEME_WHITE);
  } else {
    Board.Display.drawCircle(cx, cy, r, isConnected ? THEME_GREEN : THEME_RED);
  }
}
...
void drawPageIndicators() {
  ...
  for (int i = 0; i < totalPages; i++) {
    ...
    int indicatorX = 410 + i * 42;  // Positioned in right panel. Pitch 42
                                    // (was 50): with 7 pages the old pitch put
                                    // square 7 at x 710..740, colliding with
                                    // the HUD connection dot at (720,225);
                                    // 42 ends the row at x=692 instead.
    int indicatorY = 210;
    int indicatorSize = 30;
    
    // Filled square
    Board.Display.fillRect(indicatorX, indicatorY, indicatorSize, indicatorSize, color);
    
    // Border for active page
    if (i == currentPage) {
      Board.Display.drawRect(indicatorX - 2, indicatorY - 2, 
                         indicatorSize + 4, indicatorSize + 4, THEME_YELLOW);
    } else {
      Board.Display.drawRect(indicatorX, indicatorY, 
...
// PHYSICAL panel coordinates: page 5 is fullscreen, and the synopsis is set as
// a centred measure rather than edge-to-edge text. 40 characters at size 4
// (24 px advance, 32 px glyph) is 960 px, leaving equal 160 px margins — a
// comfortable reading width, where a full-bleed 50-character line forces the
// eye to travel the whole 1280 px panel on every return sweep.
//
// The column is repainted in place on every scroll step (each line is padded
// to SYN_CHARS and printed with an explicit background), so the margins are
// never written to and stay black between full redraws.
static const int SYN_CHARS  = 40;   // 960 px at 24 px per char, size 4
static const int SYN_X      = 160;  // (1280 - 960) / 2 — centred column
static const int SYN_TOP    = 110;
static const int SYN_LINEH  = 40;   // 32 px glyph at size 4 + 8 px leading
static const int SYN_VIS    = 13;   // visible lines: (630 - 110) / 40
...
void drawGameInfoSynopsis() {
  ...
  for (int i = 0; i < SYN_VIS; i++) {
    ...
    Board.Display.setCursor(SYN_X, SYN_TOP + i * SYN_LINEH);
    Board.Display.print(l);
  }
}
...
// -----------------------------------------------------------------------------
// drawGameInfoPageIndicator() — "1/2>>" / "<<2/2" toggle at the top-right of
// the page-5 title row. Same geometry and feedback contract as page 6's
// drawRAPageIndicator(): size 3, right-aligned to x=1250, white while pressed.
// -----------------------------------------------------------------------------
void drawGameInfoPageIndicator(bool pressed) {
  if (!currentMeta.loaded || currentMeta.synopsis.length() == 0) return;
  const char* ind = (gameInfoSubPage == 0) ? "1/2>>" : "<<2/2";
  int x = 1250 - (int)strlen(ind) * 18;    // right-aligned, size 3 = 18 px/char
  Board.Display.setTextWrap(false);
  Board.Display.setTextColor(pressed ? THEME_WHITE : THEME_CYAN, THEME_BLACK);
  Board.Display.setTextSize(3);
  Board.Display.setCursor(x, 24);
...
// -----------------------------------------------------------------------------
// displayGameInfo() — page 5 (GAME INFO / NOW PLAYING), FULLSCREEN
//
...
//
// Like page 6, this page owns the whole 1280x720 panel: the dispatch in
// updateDisplay() paints the shared chrome (nav strip + page dots) once via
// backgroundLoaded, and everything here is drawn in PHYSICAL coordinates on
// Board.Display, clearing only the content region above the strip.
// -----------------------------------------------------------------------------
void displayGameInfo() {
  // ========== CLEAR CONTENT AREA ONLY (the strip chrome persists) ==========
  Board.Display.fillRect(0, 0, 1280, 646, THEME_BLACK);
  ...
  if (haveGame && !currentMeta.loaded &&
    ...
    if (rd.available && rd.hashCalculated && rd.crc32.length() > 0) {
      ...
      Board.Display.print("FETCHING GAME INFO...");
      GameMeta m;
  ...
  // Same title-row geometry as page 6 (size 4 from x=30, rule at y=56/57), so
  // the two fullscreen pages read as one family.
  String title = haveGame ? currentGame : String("NO GAME LOADED");
  const int titleChars = 44;   // 1056 px from x=30; the indicator owns x>=1160
  ...
  Board.Display.setTextSize(4);
  Board.Display.setCursor(30, 16);
  Board.Display.print(titleShown);
  Board.Display.drawFastHLine(30, 56, 1220, THEME_CYAN);
  Board.Display.drawFastHLine(30, 57, 1220, THEME_CYAN);
  ...
  if (!haveGame || !currentMeta.loaded) {
    ...
    Board.Display.setTextColor(THEME_GRAY, THEME_BLACK);
    Board.Display.setTextSize(4);
    Board.Display.setCursor(40, 130);
    Board.Display.print(haveGame ? "NO METADATA AVAILABLE" : "LOAD A GAME ON THE MISTER");
    if (haveGame) {
      Board.Display.setTextSize(3);
      Board.Display.setCursor(40, 200);
      Board.Display.print("(game not identified on ScreenScraper)");
    }
    return;
  }
  ...
  // No frame: the arrows carry the affordance. The hit target is the whole
  // content band (see the page-5 branch in handleTouch).
  drawGameInfoPageIndicator(false);

  if (gameInfoSubPage == 0) {
    // ---- SUBPAGE 1/2: metadata fields -----------------------------------------
    // Label column (cyan, size 3) at x=40: "PLAYERS" is 126 px wide, so it
    // clears the value column at GI_VAL_X=260. Values are size 4 — one step up
    // from the labels, which gives the row its hierarchy and is the "slightly
    // larger" text the fullscreen width paid for.
    ...
    // band between the title rule (y=57) and the nav strip (y=646).
    ...
    const int BAND_TOP = 90, BAND_BOT = 630;   // usable content band
    const int ROW_STEP = 84, ROW_H = 32;       // row pitch, glyph height at size 4
    ...
    for (int r = 0; r < 6; r++) {
      ...
      // The label is one size smaller, so it is nudged down by half the glyph
      // difference ((32-24)/2 = 4) to sit centred against its value.
      Board.Display.setTextColor(THEME_CYAN, THEME_BLACK);
      Board.Display.setTextSize(3);
      Board.Display.setCursor(40, y + 4);
      Board.Display.print(rows[r].label);
      ...
      Board.Display.setTextColor(THEME_WHITE, THEME_BLACK);
      Board.Display.setTextSize(4);
      Board.Display.setCursor(GI_VAL_X, y);
      Board.Display.print(v);
    ...
    Board.Display.setTextColor(THEME_CYAN, THEME_BLACK);
    Board.Display.setTextSize(2);
    Board.Display.setCursor(SYN_X, 74);   // caption aligns with the column
    Board.Display.print("SYNOPSIS");
    drawGameInfoSynopsis();
  }
}
...
bool displayCoreImageCentered(String imagePath) {
  ...
  // all before.
  uint8_t *buffer = (uint8_t*)heap_caps_malloc(fileSize, MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
  if (buffer) {
